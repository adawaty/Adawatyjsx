...
function merge(base, patch) {
  ...
  for (const [k, v] of Object.entries(patch)) {
    ...
    else if (v && typeof v === "object" && base[k] && typeof base[k] === "object") out[k] = merge(base[k], v );
    else out[k] = v;
  }
  return out;
}
...
function localizeArray(lang, base, table) {
  if (lang === "en") return base;
  const patchTable = _optionalChain([(patches ), 'access', _ => _[lang], 'optionalAccess', _2 => _2[table === _optionalChain([(patches ), 'access', _3 => _3[lang], 'optionalAccess', _4 => _4.services]) ? "services" : ""]]);
  return base.map((item) => {
    const key = _nullishCoalesce(item.id, () => ( item.slug));
...
export function getServices(lang) {
  ...
  const table = _optionalChain([(patches ), 'access', _5 => _5[lang], 'optionalAccess', _6 => _6.services]) ;
...
export function getIndustries(lang) {
  ...
  const table = _optionalChain([(patches ), 'access', _7 => _7[lang], 'optionalAccess', _8 => _8.industries]) ;
  if (!table) return industriesEn;
  return industriesEn.map((x) => merge(x, table[x.id]));
}

export function getPersonas(lang) {
  if (lang === "en") return personasEn;
  const table = _optionalChain([(patches ), 'access', _9 => _9[lang], 'optionalAccess', _10 => _10.personas]) ;
  if (!table) return personasEn;
  return personasEn.map((x) => merge(x, table[x.id]));
}

export function getSolutions(lang) {
  if (lang === "en") return solutionsEn;
  const table = _optionalChain([(patches ), 'access', _11 => _11[lang], 'optionalAccess', _12 => _12.solutions]) ;
  if (!table) return solutionsEn;
  return solutionsEn.map((x) => merge(x, table[x.id]));
}

export function getAudiences(lang) {
  if (lang === "en") return audiencesEn;
  const table = _optionalChain([(patches ), 'access', _13 => _13[lang], 'optionalAccess', _14 => _14.audiences]) ;
  if (!table) return audiencesEn;
  return audiencesEn.map((x) => merge(x, table[x.id]));
}


export function getCaseStudyBySlug(lang, slug) {
  return getCaseStudies(lang).find((c) => c.slug === slug);
}

export function getCaseStudies(lang) {
  if (lang === "en") return caseStudiesEn;
  const table = _optionalChain([(patches ), 'access', _15 => _15[lang], 'optionalAccess', _16 => _16.caseStudies]) ;
